...
| Meta | `muse-voice-transcribe-1.0`, streaming or recording | `muse-spark-1.3`, minimal reasoning | Experimental while recording-mode acceptance remains unresolved; streaming and writing returned usable text on the TCL |
...
- **After I speak** sends one mono WAV per finished phrase. Upload bytes and server events drive the progress display. This mode's first live check failed; it remains unavailable to that account/configuration until its check succeeds.
...
- **Preferred languages** biases recognition toward any combination of English, German, Spanish and French. Leave all unselected for automatic detection, or use the main selected language as a hint. A hint is not proof of the detected language and does not force translation.
- **Use my vocabulary** optionally sends up to 100 saved terms. This is initially off; a hint does not guarantee the spelling.
...
| Meta recording mode, English/German hints | FAIL | The first actual recording check returned a provider failure. No verification or API permission was granted. Diagnosis requires another live check; voice testing is paused at the user's request. |
...
Source refinements after that installed build correct fast-turn pre-roll overlap, pending-work progress and failed-check preservation. Their final on-phone verification remains pending. Local HTTP fixtures establish contract handling and failure/cancellation behavior only.
