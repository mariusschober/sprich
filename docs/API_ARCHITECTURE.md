...
## Additional P0/P1 2026-09-02 ec493f7 (at-most-once, FIFO, isolation, privacy, security)
- **Editor exactly-once:** `CompositionManager` single `commitText` no retry on `false` (hostile editor append+return false would duplicate), `FieldSessionController.CommitResult` typed, `SprichIME.commitFinalText`/`applyFinalText` never retry `EditorRejected`, `deleteLastWord` single `deleteSurroundingText`. Partials IME-local (no external `setComposingText`), final exactly once, `HelloHello` impossible – `CompositionAdversarialTest` 7 editors + ambiguous final.
- **FIFO Stop/Endpoint:** `pendingChannel(4)` single actor for `ENDPOINT`/`USER_STOP`/30s/explicit; `USER_STOP` freezes current PCM (`size>8000`) and `enqueuePending` after earlier, `stopRequested` delays `sessionGeneration++` until `queueDepth==0` → `Idle`; `finalizeOnce` never bypasses queue; `StopEndpointRaceTest` 7 races FIFO no loss.
- **Isolation:** `failUtteranceScoped` (EditorRejected/timeout/refinement/blank/stale) keeps `Listening` without clearing B's PCM; `failSession` only for mic/field/service/corruption; actor outer `catch` → `failUtteranceScoped`; `UtteranceIsolationTest` 5 cases.
- **Cancellation never → cloud:** `LocalApiFallback` `catch CancellationException → throw`, `LocalTranscriptionCoordinator` LID/Fast/Canary respect cancellation, `tryRemote` `catch CancellationException → throw`; `CancellationNoFallbackTest` 6 cases `remote.calls==0` for WINDOW_HIDDEN/FIELD_LOST/SERVICE_DESTROYED/INPUT_RESTARTED.
- **Redirect block:** `sharedHttpClient`/`OpenAiCompatible*Provider` `followRedirects(false)` `followSslRedirects(false)` (pool via `newBuilder()`), 3xx → `Http(Redirect blocked)`, `EndpointValidator` central HTTPS/no userinfo/valid host (debug localhost), `RedirectBlockingTest` 3 cases `307→B 0`.
- **Local-cold:** `SprichIME.onCreate` reads `prefs.transcriptionMode.first()` before load; `API_PRIMARY` → 0 LID/Fast/Canary loads until lazy `transcribeSnapshot` on remote failure.
- **Episode backpressure:** `handleAudioChunk` `suppressEpisode` marks entire VAD episode suppressed while `catchingUp`, counted once, tail never captured, cleared on clean `SILENCE/LONG_SILENCE/UTTERANCE_END`.

## Privacy
- Local only + Off: network models only.
- API STT: audio sent directly to provider.
- Refinement: transcript text sent directly.
- Both: both disclosures. Always: Sprich does not provide/proxy/receive key, billing is provider-direct. Dynamic UI computes string from `transcriptionMode` + `refinementMode`. Debug WAV capture truthfully distinguishes “Never” vs “Debug capture enabled” (now `noBackupFilesDir/sprich_replay` never backed up per `backup_rules.xml`/`data_extraction_rules.xml`, paired WAV+`.meta` deletion, `Clear local data` deletes harness).

## Tests & Benchmark
- Mock matrix: `RemoteSttMatrixTest` covers success (local 0), failure→fallback, local success no remote, blank→remote, typed failures. `MockWebServerProviderTest` covers real OkHttp success/401/403/404/429/500/malformed/HTML/oversized/timeout/disconnect/cancellation/auth redaction/multipart/language; refinement valid/empty/malformed/oversized/401/429/500/timeout/cancellation; pooled client & redaction. `CoordinatorExtendedTest` covers ApiPrimary isolation, language propagation Known(DE), fallback idle/lazy, vocab relevant, command-safety, HTTPS, redaction. `ApiSecretStoreTest` covers save/load/remove, no DataStore plaintext, fail-closed, hasSecret=decryptable. `UtteranceCollectorStrictBoundTest`, `RouteFreezeTest`. **New:** `CompositionAdversarialTest` 7 (6 editors + ambiguous final), `StopEndpointRaceTest` 7 (FIFO races), `UtteranceIsolationTest` 5 (A fails B continues), `CancellationNoFallbackTest` 6 (zero remote on cancel), `RedirectBlockingTest` 3 (307→B 0).
