...
fun SettingsScreen(onBack: () -> Unit, onBenchmark: () -> Unit, onVocab: () -> Unit = {}) {
    ...
    // Use the process-wide manager so Home, the IME, and Settings observe the same model state.
    val modelManager = app.modelManager
    val downloadManager = remember(modelManager) { DownloadManager(context, modelManager) }
    ...
    val sttMode by prefs.sttModeRaw.collectAsState(initial = "local")
    val aiEnabled by prefs.aiEnabled.collectAsState(initial = false)
    val canaryStatus by modelManager.canaryStatus.collectAsState()

    val networkSummary = when {
        aiEnabled || sttMode == "remote" -> "Cloud processing enabled"
        sttMode == "fallback" -> "Local first; cloud fallback enabled"
        else -> "Model downloads only"
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            SettingsToggle(
                "Instant Dictation",
                "Start when a text field is focused",
                instant,
            ) { scope.launch { prefs.setInstantMode(it) } }
            LanguageRow(language) { scope.launch { prefs.setLanguage(it) } }
            ModelSection(
                engine = engine,
                ...
                onDownloadCanary = {
                    scope.launch {
                        try {
                            downloadManager.downloadCanary()
                        } catch (_: Exception) {
                            // DownloadManager exposes failure through ModelStatus.
                        }
                    }
                },
                onDeleteCanary = { scope.launch { modelManager.deleteCanary() } },
            ...
            SettingsRow("Network use", networkSummary) {}
            SettingsRow(
                "Clear local data",
                "",
                actionLabel = "Clear",
...
private fun SettingsRow(
    ...
    val rowModifier = if (actionLabel == null) {
        Modifier
            .fillMaxWidth()
...
private fun LanguageRow(current: Language, onSelect: (Language) -> Unit) {
    Column {
        ...
        Text(
            "Choose the language you will speak. Automatic detection is not supported by the current local model.",
        ...
        FlowRow(
            ...
            listOf(
                ...
                Language.DE to "Deutsch",
                Language.ES to "Español",
                Language.FR to "Français",
...
private fun ModelSection(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        ModelCardAdvanced(
            title = "Canary 180M Flash",
            sub = "On-device · about 147 MB",
            desc = "Local English, German, Spanish, and French transcription.",
            selected = engine == EngineType.ACCURATE,
            status = canaryStatus,
            onClick = { onSelect(EngineType.ACCURATE) },
            onDownload = onDownloadCanary,
            onDelete = onDeleteCanary,
            onCancel = onCancel,
            totalMb = 147,
        )
    }
        ...
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            Row(
                ...
                when (status) {
                    ...
                    ModelStatus.Ready -> Row(
                        verticalAlignment = androidx.compose.ui.Alignment.CenterVertically,
                        horizontalArrangement = Arrangement.spacedBy(4.dp),
                    ) {
                        if (selected) {
                            Badge { Text("Active", style = MaterialTheme.typography.labelSmall) }
                        } else {
                    ...
                    ModelStatus.Verifying -> CircularProgressIndicator(modifier = Modifier.size(16.dp), strokeWidth = 2.dp)
                    is ModelStatus.Failed -> Text(
                        status.error.take(24),
                        style = MaterialTheme.typography.labelSmall,
                        color = MaterialTheme.colorScheme.error,
                    )

                    ModelStatus.NotDownloaded -> TextButton(onClick = onDownload) {
            ...
            if (status is ModelStatus.Downloading) {
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Text(
                        "${status.bytes / 1024 / 1024} / ${if (status.total > 0) status.total / 1024 / 1024 else totalMb} MB",
                        style = MaterialTheme.typography.labelSmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                    )
                }
...
private fun BackupSttSection(prefs: Preferences) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Text(
            "OpenAI-compatible endpoint. Works with Grok (x.ai), Groq, Wizper via fal proxies, or any custom gateway.",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            FilterChip(
                selected = modeRaw == "local",
                onClick = { scope.launch { prefs.setSttMode(Preferences.SttMode.LOCAL) } },
                label = { Text("Local only", style = MaterialTheme.typography.labelSmall) },
            )
            FilterChip(
                selected = modeRaw == "fallback",
                onClick = { scope.launch { prefs.setSttMode(Preferences.SttMode.FALLBACK) } },
                label = { Text("Local→Cloud fallback", style = MaterialTheme.typography.labelSmall) },
            )
            FilterChip(
                selected = modeRaw == "remote",
                onClick = { scope.launch { prefs.setSttMode(Preferences.SttMode.REMOTE) } },
                label = { Text("Cloud primary", style = MaterialTheme.typography.labelSmall) },
            )
        }
        if (modeRaw != "local") {
            Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                AssistChip(
                    onClick = {
                        ...
                        scope.launch {
                            prefs.setSttBaseUrl(baseUrl)
                            prefs.setSttModel(model)
                        }
                    },
                    label = { Text("Grok x.ai", style = MaterialTheme.typography.labelSmall) },
                )
                AssistChip(
                    onClick = {
                    ...
                    label = { Text("Groq Whisper", style = MaterialTheme.typography.labelSmall) },
                )
                AssistChip(
                    onClick = {
                        scope.launch {
                    ...
                    label = { Text("Custom / fal", style = MaterialTheme.typography.labelSmall) },
                )
            }
            OutlinedTextField(
                value = baseUrl,
                ...
                modifier = Modifier.fillMaxWidth(),
                trailingIcon = {
                    TextButton(onClick = { scope.launch { prefs.setSttBaseUrl(baseUrl) } }) { Text("Save") }
                },
            )
            OutlinedTextField(
                value = apiKey,
                ...
                modifier = Modifier.fillMaxWidth(),
                trailingIcon = {
                    TextButton(onClick = { scope.launch { prefs.setSttApiKey(apiKey) } }) { Text("Save") }
                },
            )
            OutlinedTextField(
                value = model,
                ...
                modifier = Modifier.fillMaxWidth(),
                trailingIcon = {
                    TextButton(onClick = { scope.launch { prefs.setSttModel(model) } }) { Text("Save") }
                },
            )
        }
    }
        ...
        if (enabled) {
            Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                AssistChip(
                    ...
                    label = { Text("Gemini fast", style = MaterialTheme.typography.labelSmall) },
                )
                AssistChip(
                    onClick = {
                    ...
                    label = { Text("GPT mini", style = MaterialTheme.typography.labelSmall) },
                )
                AssistChip(
                    onClick = {
                        baseUrl = "https://api.x.ai/v1"
                    ...
                    label = { Text("Grok mini", style = MaterialTheme.typography.labelSmall) },
                )
            }
            OutlinedTextField(
                value = baseUrl,
                ...
                modifier = Modifier.fillMaxWidth(),
                trailingIcon = {
                    TextButton(onClick = { scope.launch { prefs.setAiBaseUrl(baseUrl) } }) { Text("Save") }
                },
            )
            OutlinedTextField(
                value = apiKey,
                ...
                modifier = Modifier.fillMaxWidth(),
                trailingIcon = {
                    TextButton(onClick = { scope.launch { prefs.setAiApiKey(apiKey) } }) { Text("Save") }
                },
            )
            OutlinedTextField(
                value = model,
                ...
                modifier = Modifier.fillMaxWidth(),
                trailingIcon = {
                    TextButton(onClick = { scope.launch { prefs.setAiModel(model) } }) { Text("Save") }
                },
            )
        }
    }
