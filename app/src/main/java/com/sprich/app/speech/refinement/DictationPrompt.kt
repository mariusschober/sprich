...
object DictationPrompt {
    ...
    fun system(mode: RefinementMode, variant: Variant = Variant.CONTRACT, whisperMode: Boolean = false): String {
        ...
        val instructions = if (mode == RefinementMode.CORRECT) base + " Preserve fillers and repetition in this mode; correct spelling and punctuation only." else base
        return if (whisperMode) instructions + " The recording was whispered. Render it as ordinary written text. Never add whisper stage directions or invent inaudible words; preserve uncertain wording." else instructions
    }
