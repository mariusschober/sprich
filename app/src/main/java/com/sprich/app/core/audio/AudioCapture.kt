...
class AudioCapture(
    ...
    fun start(
        ...
        try {
            ...
            // Low-latency: 2x min is optimal; avoid forcing 1s buffer
                ...
                try {
                    while (isCapturing.get() && rec.recordingState == AudioRecord.RECORDSTATE_RECORDING) {
                        ...
                        if (n > 0) {
                            ...
                            ringBuffer.write(copy)
                            val frameRms = rms(copy)
                            if (!loggedFirstFrame) {
                                loggedFirstFrame = true
                                Log.i(TAG, "first audio frame samples=$n rms=$frameRms")
                            }
                            healthFrames++
                            maxHealthRms = maxOf(maxHealthRms, frameRms)
                            if (healthFrames >= 16) {
                                Log.i(TAG, "audio health frames=$healthFrames maxRms=$maxHealthRms")
                                healthFrames = 0
                                maxHealthRms = 0f
                            }
                            try { onChunk(copy, now) } catch (e: Exception) { Log.w(TAG, "onChunk error", e) }
